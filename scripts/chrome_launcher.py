    ...
    try:
        ...
        if sys.platform == "darwin":
            return os.path.join(os.path.expanduser("~"),
                                "Library", "Application Support", PROFILE_DIR_NAME)
...
def is_port_open(port: int, host: str = "localhost") -> bool:
    """Check if a TCP port is accepting connections. Uses IPv6 on macOS since Chrome binds to [::1]."""
    # On macOS Chrome binds to IPv6 [::1], not IPv4 0.0.0.0
    # AF_INET6 works for both IPv4 and IPv6 addresses on macOS
    try:
