...
CDP_HOST = "localhost"  # macOS Chrome binds to IPv6 [::1], localhost resolves both
CDP_PORT = 9223
...
class XiaohongshuPublisher:
    ...
    def _click_publish(self, scheduled: bool = False):
        ...
        # Pre-flight: check for validation errors (e.g. title too long)
        pre_error = self._evaluate("""
            (() => {
                const errorEls = document.querySelectorAll(
                    '.el-message--error, .error-tip, [class*="error"], .toast-error, .warning-text'
                );
        ...
        if pre_error:
            raise CDPError(
                f"Validation error before publish: {pre_error}"
            )
        ...
        # Use JS click as primary, CDP mouse as fallback
        js_clicked = self._evaluate(f"""
            (() => {{
                const selectors = [
                    {json.dumps(SELECTORS["publish_button"])},
                    "button.publishBtn",
                ...
                for (const btn of buttons) {{
                    ...
                    if ((text === "发布" || text === "定时发布") && !btn.disabled) {{
                        btn.click();
                        return true;
                    }}
                }}
        ...
        if not js_clicked:
            # Fallback to CDP mouse click
            print("[cdp_publish] JS click failed, falling back to CDP mouse click...")
            self._click_mouse(cx, cy)

        print("[cdp_publish] Publish button clicked.")

        # Wait and verify publish actually succeeded
        ...
        for attempt in range(3):
            result = self._evaluate("""
                (() => {
                    ...
                    // Success: found note link in success dialog
                    const links = document.querySelectorAll('a[href*="xiaohongshu.com/explore"]');
                    if (links.length > 0) {
                        return { status: 'success', url: links[0].href };
                    }

                    // Success: success message visible
                    const successEls = document.querySelectorAll(
                        '.el-message--success, [class*="success"], .publish-success'
                    );
                    for (const el of successEls) {
                        const text = (el.innerText || '').trim();
                        if (text && (text.includes('发布成功') || text.includes('审核'))) {
                            return { status: 'success', message: text };
                        }
                    }

                    // Error: validation error appeared after click
                    const errorEls = document.querySelectorAll(
                        '.el-message--error, .error-tip, [class*="error"], .toast-error'
                    );
                    for (const el of errorEls) {
                        const text = (el.innerText || el.textContent || '').trim();
                        if (text && text.length > 2 && text.length < 200) {
                    ...
                    // Still on publish page, no clear signal
            ...
            if result and isinstance(result, dict):
                ...
                if status == "success":
                    ...
                    note_link = result.get("url") or result.get("message")
                    print(f"[cdp_publish] Publish verified: {note_link or 'success'}")
                    break
                elif status == "error":
                    raise CDPError(
                        f"Publish failed: {result.get('message', 'unknown error')}"
                    )
        ...
        if not publish_verified:
            print(
                "[cdp_publish] WARNING: Could not verify publish success. "
                "Page is still on the publish editor. "
