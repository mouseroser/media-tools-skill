...
from cdp_publish import XiaohongshuPublisher, CDPError, CDP_PORT
...
def main():
    ...
    # Xiaohongshu title limit: max 20 characters
    XHS_TITLE_MAX = 20
    if len(title) > XHS_TITLE_MAX:
        print(
            f"[pipeline] ERROR: Title is {len(title)} chars, "
            f"but Xiaohongshu allows max {XHS_TITLE_MAX}. "
            f"Title: \"{title}\"",
            file=sys.stderr,
        )
        sys.exit(2)
