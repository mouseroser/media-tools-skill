...
**media-tools = 自媒体执行（工具层），wemedia = 自媒体运营（**端到端负责**）**
...
| 用途 | 路径 | 写入者 | 读者 |
|------|------|--------|------|
| 内容草稿 | `~/.openclaw/workspace/intel/collaboration/media/wemedia/drafts/{A\|B\|C}/{标识}.txt` | wemedia agent | main / media-tools |
| 配图 | `~/.openclaw/workspace/intel/collaboration/media/images/{A\|B\|C}/{标识}_sq.jpg` | wemedia subagent | main / media-tools |
...
> wemedia subagent 端到端负责：读自己写的草稿、自己生成配图、自己调用发布脚本。
